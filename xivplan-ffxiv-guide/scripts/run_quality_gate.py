...
def render_markdown(summary: dict[str, Any], results: list[dict[str, Any]]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Phase G/P/Q/R visual quality score across context, density, labels, flow, layers, aesthetics, step story, enemy identity, enemy icons, and party job/role identity. Severe issues fail; review issues are reported but do not block.",
        ]
    )
    return "\n".join(lines) + "\n"
