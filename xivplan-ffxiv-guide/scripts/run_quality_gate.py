...
def render_markdown(summary: dict[str, Any], results: list[dict[str, Any]]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Phase F/O storyboard coverage, required step metadata, and observation / assignment / movement / resolution / reset / next-read coverage when the scene is generated by `build_spec_from_solution.py`.",
            "- Phase G/P/Q/R visual quality score across context, density, labels, flow, layers, aesthetics, step story, enemy identity, enemy icons, and section-aware party identity. Severe issues fail; review issues are reported but do not block.",
            "- Phase V route/range semantics: movement endpoints, resolve rounds, endpoint-snap declarations, AoE source/targets/timing/intent, real resolve geometry, and undeclared current-danger crossing.",
            "- Phase X status assignments: buff/debuff role bindings, readable upper-left player overlays, correct party anchors, and documented fallback status badges.",
        ]
    )
    return "\n".join(lines) + "\n"
