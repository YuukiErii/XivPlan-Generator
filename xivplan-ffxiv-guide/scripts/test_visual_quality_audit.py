...
def good_scene() -> dict:
    ...
    party = [{"type": "party", "role": role, "x": x, "y": y, "width": 30, "height": 30, "id": index} for index, (role, x, y) in enumerate(roles, start=10)]
    markers = [
    ...
    base = markers + [{"type": "enemy", "name": "Boss", "x": 0, "y": 0, "radius": 45, "id": 5}] + party
    return {
        "name": "visual quality smoke",
        "arena": {"shape": "circle", "width": 600, "height": 600, "padding": 120, "grid": {"type": "radial"}},
        "metadata": {"source": "build_spec_from_solution.py", "storyboard_generator": "phase-f-v2"},
        "steps": [
            {
                "title": "1 观察",
...
def main() -> int:
    ...
    assert long_flow_result["status"] == "PASS", long_flow_result
    ...
    print("OK: visual quality audit covers severe failures, label/title avoidance, movement flow, and semantic long-flow density")
    return 0
